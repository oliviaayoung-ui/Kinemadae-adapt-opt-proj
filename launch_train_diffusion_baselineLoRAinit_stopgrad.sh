#!/bin/bash
# Fresh start: B-adaptive + align_projection L2 + diffusion loss (flow matching, REPA-E 방식)
# Base: launch_train_lora_bn_clamp1e7_b_adaptive_proj_only_fresh.sh
# ADDED (= stage2 rm57nmln 동일):
#   --use_diffusion_loss                       : flow matching diffusion loss (z_cat detach → VAE 무관)
#   --diffusion_loss_weight 1.0                : REPA diffusion main 1.0
#   --diffusion_unfreeze_zprior_patchify       : z_prior patchify 학습 (rm57nmln)
#   --diffusion_unfreeze_block_norms_mod       : block norm+mod unfreeze (rm57nmln)
...
$PY -m torch.distributed.run --nproc_per_node=$NUM_GPUS --standalone train_causalvae_geoprior_dit_align.py \
    --exp_name "kinemadae_lora_bn_clamp1e7_align40_bs${BS}${EXP_SUFFIX}_b_proj_diffusion_baselineLoRAinit_stopgrad" \
