#!/bin/bash
# Stage1 VAE align (ablation: LoRA + zmain BN 제거)
# Base: launch_train_stage1_bn_lora_teacherfrozen.sh 에서
#   - LoRA 제거 (--use_lora / --lora_rank / --lora_target_modules)
#   - zmain BN 제거 (--normalize_zmain_bn / --bn_momentum / --zmain_bn_init)
#   - teacher_frozen_pretrained 제거 (LoRA 없으면 teacher = pretrained Wan 자동 → no-op 이라 불필요)
# 목적: LoRA+BN 의 기여 분리용 ablation. 순수 VAE align (encoder/decoder/projection 만 학습).
# 유지: align40 + cosine + b_adaptive + align_projection + z_prior patchify freeze.
...
$PY -m torch.distributed.run --nproc_per_node=$NUM_GPUS --standalone train_causalvae_geoprior_dit_align.py \
    --exp_name "kinemadae_stage1_align40_bs${BS}${EXP_SUFFIX}_b_proj_noLoRA_noBN" \
