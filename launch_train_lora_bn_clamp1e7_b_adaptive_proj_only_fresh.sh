#!/bin/bash
# Fresh start version: B-adaptive + align_projection L2 (mid=16) — noise subsample 원복 (= off)
# Base: launch_train_lora_bn_clamp1e7_b_adaptive_proj_zprior_fresh.sh
# CHANGED: --teacher_subsample_noise_mode z_prior → off (= 일반 randn)
...
$PY -m torch.distributed.run --nproc_per_node=$NUM_GPUS --standalone train_causalvae_geoprior_dit_align.py \
    --exp_name "kinemadae_lora_bn_clamp1e7_align40_bs${BS}${EXP_SUFFIX}_b_proj_only_fresh" \
