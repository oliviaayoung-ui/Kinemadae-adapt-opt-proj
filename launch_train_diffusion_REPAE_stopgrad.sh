#!/bin/bash
# [REPA-E 정석] align forward 시 DiT freeze → align grad VAE only (충돌 제거). diffusion 정상.
...
$PY -m torch.distributed.run --nproc_per_node=$NUM_GPUS --standalone train_causalvae_geoprior_dit_align.py \
    --exp_name "kinemadae_lora_bn_clamp1e7_align40_bs${BS}${EXP_SUFFIX}_b_proj_diffusion_REPAE_stopgrad" \
