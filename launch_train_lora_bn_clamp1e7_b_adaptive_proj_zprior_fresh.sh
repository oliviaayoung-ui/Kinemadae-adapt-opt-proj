#!/bin/bash
# Fresh start version: B-adaptive + align_projection L2 (mid=16) + teacher_subsample z_prior
# Base: launch_train_lora_bn_clamp1e7_b_adaptive_proj_zprior.sh
# CHANGED: --resume_from_checkpoint + --resume_ckpt_grad_accum 제거 (= fresh start)
...
$PY -m torch.distributed.run --nproc_per_node=$NUM_GPUS --standalone train_causalvae_geoprior_dit_align.py \
    --exp_name "kinemadae_lora_bn_clamp1e7_align40_bs${BS}${EXP_SUFFIX}_b_proj_zprior_fresh" \
