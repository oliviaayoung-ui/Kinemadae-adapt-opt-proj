#!/bin/bash
# [ABLATION] diffusion_only: align_loss ×0 (align grad 차단). z_prior diffusion loss 가 align 충돌 때문인지 격리.
...
$PY -m torch.distributed.run --nproc_per_node=$NUM_GPUS --standalone train_causalvae_geoprior_dit_align.py \
    --exp_name "kinemadae_lora_bn_clamp1e7_align40_bs${BS}${EXP_SUFFIX}_b_proj_diffusion_ONLY_ablation" \
