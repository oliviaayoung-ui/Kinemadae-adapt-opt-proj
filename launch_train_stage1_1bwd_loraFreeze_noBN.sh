#!/bin/bash
# Stage1 VAE align + zmain BN + LoRA + TEACHER FROZEN (diffusion 없음, NOT end-to-end)
# Base: launch_train_diffusion_baselineLoRAinit_stopgrad_teacherfrozen.sh 에서
#   - diffusion loss 계열 전부 제거 (use_diffusion_loss / unfreeze_* / align_stop_grad)
#   - init_lora_safetensors 제거 → LoRA 는 Wan I2V pretrained random init (lora_B=0)
#   - teacher_frozen_pretrained 유지 (디폴트, align target = pretrained Wan I2V 고정)
# 동작: LoRA 가 align loss 로 학습 (align_stop_grad 없음) → student feature 를 frozen pretrained teacher 에 정렬.
