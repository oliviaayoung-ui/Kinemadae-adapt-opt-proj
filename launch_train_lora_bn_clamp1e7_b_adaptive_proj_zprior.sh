#!/bin/bash
# B-adaptive + align_projection (per-block residual) + teacher_subsample_noise mode=z_prior
# Base: launch_train_lora_bn_clamp1e7_b_adaptive.sh
# Added:
#   --use_align_projection                  : block 별 conv3d residual (zero init), optimizer 등록 적용됨
#   --teacher_subsample_noise_mode z_prior  : z_prior 부분만 teacher noise subsample (= 옵션 C)
...
$PY -m torch.distributed.run --nproc_per_node=$NUM_GPUS --standalone train_causalvae_geoprior_dit_align.py \
    --exp_name "kinemadae_lora_bn_clamp1e7_align40_bs${BS}${EXP_SUFFIX}_b_proj_zprior" \
    ...
    --align_num_blocks 40 \
    --use_b_adaptive \
    --resume_from_checkpoint /NHNHOME/WORKSPACE/0226010404_A/CVLAB/CVLAB2/jeeyoung/Kinemadae-adaptive-Bfix/results/kinemadae_lora_bn_clamp1e7_align40_bs4_fresh_seed1234_nw5_b_adaptive_4gpu-lr8.00e-05-bs4-rs256-sr2-fr17/checkpoint-20000.ckpt \
    --resume_ckpt_grad_accum 8 \
    --log_adaptive_weight \
    --seed 1234 \
    ...
    --teacher_subsample_noise_mode z_prior \
