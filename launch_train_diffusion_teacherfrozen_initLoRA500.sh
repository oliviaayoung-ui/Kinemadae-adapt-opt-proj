#!/bin/bash
# Stage1 align + BN + LoRA + DIFFUSION + teacher=STEP-500 LoRA FREEZE
# Base: launch_train_diffusion_teacherfrozen_noStopgrad.sh + init_lora_safetensors(step-500).
# 핵심: --init_lora_safetensors 가 주어지면 teacher_frozen 이 자동으로 'step-500 LoRA freeze' 모드.
#   - student LoRA = step-500 으로 init → 계속 학습
#   - teacher = step-500 LoRA 고정 (teacher forward 시 LoRA off 대신 step-500 snapshot 으로 swap)
#   - diffusion: z.detach → VAE 무관. align: VAE+DiT 둘 다 (align_stop_grad 없음).
#   - block_norms 도 teacher forward 시 pretrained swap (기존 teacher_frozen 동작 유지)
# 메모리: teacher 용 LoRA snapshot +5.7GB/GPU
...
$PY -m torch.distributed.run --nproc_per_node=$NUM_GPUS --standalone train_causalvae_geoprior_dit_align.py \
    --exp_name "kinemadae_stage1_align40_bs${BS}${EXP_SUFFIX}_b_proj_diffusion_teacherfrozen_initLoRA500" \
    ...
    --init_lora_safetensors /NHNHOME/WORKSPACE/0226010404_A/CVLAB/CVLAB2/jeeyoung/Kinemadae-adaptive-Bfix/results/kinemadae_stage1_align40_bs4_b_proj_diffusion_teacherfrozen_noStopgrad-lr8.00e-05-bs4-rs256-sr2-fr17/lora_step500.safetensors \
